...
from scikeras.wrappers import KerasClassifier # Certifique-se de que a versão é 0.8.0 ou superior
...
if gpus:
    try:
        ...
        print("Modelos serão treinados/avaliados na GPU, se possível.")
    except RuntimeError as e:
        print(f"ERRO ao configurar GPU: {e}")
        print("O TensorFlow pode não usar a GPU ou pode ocorrer um erro inesperado.")
else:
    print("Nenhuma GPU detectada pelo TensorFlow.")
print("--- Fim da Verificação de GPU ---")

USAR_DADOS_BRUTOS = True
PORCENTAGEM_DADOS_USAR_TOTAL = 1.0 # 1.0 = 100% dos dados.
...
try:
    ...
    raise
SESSOES_A_CARREGAR = []
print(f"INFO: Varrendo sessões em '{CAMINHO_BASE_DADOS}'...")
for item in os.listdir(CAMINHO_BASE_DADOS):
    if item.startswith('Session') and os.path.isdir(os.path.join(CAMINHO_BASE_DADOS, item)):
        SESSOES_A_CARREGAR.append(item)
SESSOES_A_CARREGAR.sort() # Garante uma ordem consistente (Session1, Session2, ...)

if not SESSOES_A_CARREGAR:
    raise ValueError(f"ERRO CRÍTICO: Nenhuma pasta 'SessionX' encontrada em '{CAMINHO_BASE_DADOS}'. Verifique o caminho base e a estrutura das pastas.")
else:
    print(f"INFO: Sessões detectadas para carregar: {SESSOES_A_CARREGAR}")
...
INDICE_CANAL_LABEL = 31 # Mantenha 31, mas confirme na documentação do dataset
...
DROPOUT_RATES_GRID = [0.4] # Definido como 0.4 conforme solicitado
FILTROS_CONV1_GRID = [64, 128]

NOME_ARQUIVO_MODELO_SALVO = f'melhor_modelo_cnn_emg_raw_{USAR_DADOS_BRUTOS}_p{int(PORCENTAGEM_DADOS_USAR_TOTAL*100)}_s{len(SESSOES_A_CARREGAR)}.keras'
NOME_ARQUIVO_CHECKPOINT = f'modelo_checkpoint_emg_raw_{USAR_DADOS_BRUTOS}_p{int(PORCENTAGEM_DADOS_USAR_TOTAL*100)}_s{len(SESSOES_A_CARREGAR)}.keras'
...
print(f"  Sessões a Carregar: {SESSOES_A_CARREGAR}")
...
print(f"  Checkpoints serão salvos como: '{NOME_ARQUIVO_CHECKPOINT}' (durante o treinamento)")
if CAMINHO_MODELO_CARREGAR:
        ...
        for caminho_registro_sem_ext in arquivos_dat:
            try:
                ...
                for i in range(num_segmentos_no_arquivo):
                    ...
                    valid_labels_in_segment = [lbl for lbl in segmento_labels.astype(int) if lbl in DICIONARIO_CLASSES_GESTOS]

                    if len(valid_labels_in_segment) == 0:
                        continue

                    mode_result = scipy_mode(valid_labels_in_segment, keepdims=False)
                    label_segmento = mode_result.mode

                    if isinstance(label_segmento, np.ndarray) and label_segmento.size > 0:
                        label_segmento = label_segmento.item()
                    elif isinstance(label_segmento, (list, tuple)) and len(label_segmento) > 0:
...
iterador_contagem = iter(tqdm_auto(gerador_para_contagem, desc="Coletando Labels Reais"))
try:
...
if dataset_treino and not CAMINHO_MODELO_CARREGAR:
    try:
...
def criar_modelo_cnn_base(
    ...
    # Corrigido: Aumenta o número de filtros para esta camada para evitar repetição exata e aumentar capacidade
    # e ajusta o nome da ativação para clareza
    filtros_conv4 = filtros_conv3 * 2 # Aumentado para dobrar os filtros da camada anterior
    x = layers.Conv1D(filters=filtros_conv4, kernel_size=3, padding='same', name='Conv4_1')(x)
    x = layers.BatchNormalization(name='BN4_1')(x)
    x = layers.Activation(activation, name='Act4_1_Updated')(x) # Renomeado para indicar distinção
...
def salvar_checkpoint_modelo(filepath=NOME_ARQUIVO_CHECKPOINT):
    print(f"\nINFO: Configurando ModelCheckpoint para salvar em '{filepath}'")
...
def verificar_arquitetura_e_neuronios(model, X_sample=None):
    print("\n" + "="*20 + " ANÁLISE DA ARQUITETURA E NEURÔNIOS " + "="*20)
    if not model:
        print("AVISO: Modelo não disponível para análise.")
        return
    ...
    for i, layer in enumerate(model.layers):
        ...
        print(f"  Camada {i+1}: {layer.name} ({layer_type})")
        
        current_layer_config = str(layer.get_config())
        if layer_type == previous_layer_name and current_layer_config == previous_layer_config:
            print(f"    >>> ALERTA: Camada '{layer.name}' ({layer_type}) parece ser uma **repetição imediata** da camada anterior com as mesmas configurações.")
            print("    Isso pode ser intencional (e.g., residual block) ou indicar um erro na definição da arquitetura.")
        
        previous_layer_name = layer_type
        previous_layer_config = current_layer_config

    for l_type, count in layer_types.items():
        print(f"  {l_type}: {count} instâncias")
    
    if X_sample is not None:
        print("\n--- Análise da Atividade dos Neurônios (Camadas Dense) ---")
        try:
            dense_layers = [layer for layer in model.layers if isinstance(layer, layers.Dense) and 'Dense1' in layer.name]
            
            if dense_layers:
                target_layer = dense_layers[-1]
                
                intermediate_model = models.Model(inputs=model.input, outputs=target_layer.output)
                
                activations = intermediate_model.predict(X_sample)
                ...
                dead_threshold = 1e-6
                low_variance_threshold = 1e-4 # Novo limiar para baixa variação
                
                print(f"Analisando ativações da camada '{target_layer.name}' com {num_neurons} neurônios:")

                dead_neurons_count = 0
                low_variance_neurons_count = 0

                for n_idx in range(num_neurons):
                    neuron_output = activations[:, n_idx]
                    
                    if np.all(np.abs(neuron_output) < dead_threshold):
                        dead_neurons_count += 1
                    
                    # Verifica neurônios com baixa variação de saída (potencialmente inativos ou saturados)
                    if np.std(neuron_output) < low_variance_threshold:
                        low_variance_neurons_count += 1

                if dead_neurons_count > 0:
                    print(f"  >>> ALERTA: {dead_neurons_count} de {num_neurons} neurônios na camada '{target_layer.name}' podem estar **mortos** (saídas consistentemente próximas de zero).")
                    print("  Isso pode indicar uma taxa de aprendizado muito alta, inicialização inadequada ou problemas no dataset (ex: dados ruins, falta de normalização).")
                else:
                    print(f"  INFO: Nenhuma indicação de neurônios 'mortos' na camada '{target_layer.name}'.")
                
                if low_variance_neurons_count > 0:
                    print(f"  AVISO: {low_variance_neurons_count} de {num_neurons} neurônios na camada '{target_layer.name}' apresentam **baixa variação de saída** (podem estar subutilizados ou saturados).")
                else:
                    print(f"  INFO: Nenhuma indicação de neurônios com baixa variação de saída na camada '{target_layer.name}'.")

            else:
                print("INFO: Nenhuma camada Dense ou camada de ativação identificada para análise de neurônios.")

        except Exception as e:
            print(f"AVISO: Não foi possível realizar a análise de neurônios. Erro: {e}")
            import traceback
            traceback.print_exc()
    else:
        print("INFO: X_sample não fornecido. Análise da atividade dos neurônios ignorada.")

    print("="*20 + " FIM DA ANÁLISE " + "="*20)
...
if CAMINHO_MODELO_CARREGAR:
    ...
    try:
        ...
        X_sample_for_check = None
        if dataset_teste_metrics:
            try:
                for x_batch, _ in dataset_teste_metrics.take(1):
                    X_sample_for_check = x_batch.numpy()
                print(f"INFO: Usando amostra de dados de teste de shape {X_sample_for_check.shape} para análise de neurônios.")
            except Exception as e:
                print(f"AVISO: Não foi possível obter amostra de dados de teste para análise de neurônios: {e}")

        verificar_arquitetura_e_neuronios(melhor_modelo_final, X_sample=X_sample_for_check)
...
if not CAMINHO_MODELO_CARREGAR:
    # Removida a conversão para NumPy. GridSearchCV aceitará tf.data.Dataset diretamente.
    # Certifique-se de que a versão do scikeras é 0.8.0 ou superior.

    if dataset_treino is None or dataset_teste_eval is None:
        print("ERRO CRÍTICO: Datasets de treino ou de validação estão vazios. Não é possível prosseguir com GridSearchCV.")
        grid_search = None
        melhor_modelo_final = None
        history_of_best_model_refit = None
    else:
        ...
        param_grid = {
            ...
            'model__dropout_rate': DROPOUT_RATES_GRID, # Agora só 0.4
        ...
        # Ajuste dos Callbacks para o GridSearchCV
        es_callback_grid = EarlyStopping( monitor='val_loss', patience=PACIENCIA_EARLY_STOPPING, verbose=0, mode='min', restore_best_weights=True ) # Usando paciência aumentada
        rlr_callback_grid = ReduceLROnPlateau( monitor='val_loss', factor=0.2, patience=PACIENCIA_REDUCE_LR, min_lr=1e-7, verbose=0, mode='min' ) # Usando paciência aumentada e factor 0.2
        
        checkpoint_callback_grid = salvar_checkpoint_modelo(filepath=NOME_ARQUIVO_CHECKPOINT)
        callbacks_grid = [es_callback_grid, rlr_callback_grid, checkpoint_callback_grid]
        ...
        print(f"      Usando dataset_treino para treinar e dataset_teste_eval para validação.")
        print("ATENÇÃO: Esta etapa pode ser demorada!")

        t_grid_inicio = time.time()
        try:
            # Passando os tf.data.Dataset diretamente para o fit
            grid_search.fit( X=dataset_treino, y=None, validation_data=dataset_teste_eval, callbacks=callbacks_grid )
            ...
            verificar_arquitetura_e_neuronios(melhor_modelo_final, X_sample=dataset_teste_eval.take(1).map(lambda x,y: x).get_single_element().numpy())
...
if melhor_modelo_final and dataset_teste_metrics:
    ...
    for segmentos_lote, labels_reais_lote_encoded in tqdm_auto(dataset_teste_metrics, desc="Avaliando Lotes Teste"):
