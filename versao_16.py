...
# Optional: Set TensorFlow log level to see all messages, including GPU detection
# os.environ['TF_CPP_MIN_LOG_LEVEL'] = '0' # Uncomment this line to see more TF logs
...
if gpus:
    ...
    print("O script será executado na CPU. Para usar a GPU, certifique-se de que o TensorFlow-GPU, CUDA Toolkit e cuDNN estejam corretamente instalados e configurados.")
print("--- Fim da Verificação de GPU ---")
# --- End GPU Check Section ---

USAR_DADOS_BRUTOS = True
PORCENTAGEM_DADOS_USAR_TOTAL = 1.0 # 1.0 = 100% dos dados.

# TEMPORARIAMENTE DEFINIDO COMO NONE PARA FORÇAR O TREINAMENTO E SALVAMENTO DO MODELO
# Após o primeiro treinamento bem-sucedido, você pode apontar para o modelo salvo aqui.
...
# --- NOVO: Varrer todas as sessões dinamicamente ---
SESSOES_A_CARREGAR = []
...
# O nome do arquivo salvo agora refletirá as novas configurações (100% dos dados, etc.)
# É uma boa prática incluir o número de sessões no nome para clareza.
        ...
        for caminho_registro_sem_ext in arquivos_dat:
            try:
                ...
                for i in range(num_segmentos_no_arquivo):
                    ...
                    # CRÍTICO: VERIFICA SE O SEGMENTO TEM UM LABEL CONSISTENTE E CONHECIDO
                    # Filtra labels que não estão no seu dicionário ou que são NaN
                    # Note: Convert to int first, then check against DICIONARIO_CLASSES_GESTOS keys
                    ...
                    if len(valid_labels_in_segment) == 0:
                        continue # Pula se não houver labels válidos e conhecidos no segmento

                    mode_result = scipy_mode(valid_labels_in_segment, keepdims=False)
                    label_segmento = mode_result.mode

                    # Ensure label_segmento is a single scalar
                    if isinstance(label_segmento, np.ndarray) and label_segmento.size > 0:
                        label_segmento = label_segmento.item() # Use .item() for scalar NumPy arrays
...
if not CAMINHO_MODELO_CARREGAR:
    ...
    if dataset_treino:
        ...
        X_treino_list = []
        y_treino_list = []
        for x_batch, y_batch in tqdm_auto(dataset_treino, desc="Convertendo Treino para NumPy"):
    ...
    if dataset_teste_eval:
        ...
        X_val_list = []
        y_val_list = []
        for x_batch, y_batch in tqdm_auto(dataset_teste_eval, desc="Convertendo Validação para NumPy"):
